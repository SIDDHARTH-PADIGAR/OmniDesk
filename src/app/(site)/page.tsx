...
import { CLIENTS, PRICING_PLANS, PRICING_CARDS, USERS } from '../../lib/constants';
  ...
  return (
    <>
      <section className='overflow-hidden px-4 sm:px-6 mt-10 sm:flex sm:flex-col md:justify-center md:items-center'>
        <TitleSection
          pill="📓 Your Workspace, Optimized"
          title="All-in-one Solution: Boost your Productivity"
        />
        <section className="bg-white p-[2px] mt-6 rounded-xl bg-gradient-to-r from-primary to-brand-primaryBlue sm:w-[300px]">
          <Button
          ...
          {[...Array(2)].map((_, index) => (
            <div key={index} className="flex flex-nowrap animate-slide">
              {CLIENTS.map((client, i) => (
                <div key={i} className="relative w-[200px] m-20 shrink-0 flex items-center">
      ...
      <section className='px-4 sm:px-6 flex justify-center items-center flex-col relative'>
        <div className='w-[30%] blur-[120px] h-32 absolute bg-brand-primaryPurple/50 -z-10 top-22'/>
        <TitleSection title='Manage all your meetings in one place'
          subheading='Record your ideas, thoughts, and meeting notes systematically and efficiently.'
          pill='Features'/>
      ...
      <section className='relative'>
        <div className='w-full blur-[120px] h-32 absolute bg-brand-primaryPurple/50 -z-10 top-56'/>
...
<section className='mt-20 px-4 sm:px-6'>
  <TitleSection
    title='Your personalized plan awaits!'
    subheading="Discover the full range of advantages our platform offers. Choose the plan that fits your requirements and elevate your productivity to unprecedented levels."
    pill="Pricing"
  />
    ...
    {PRICING_CARDS.map((card) => (
      <CustomCard
        ...
        className={clsx('w-[300px] rounded-2xl dark:bg-black/40 background-blur-3xl relative', {
          'border-brand-primaryPurple/60': card.planType === PRICING_PLANS.proplan,
        })}
        cardHeader={
    ...
    {card.planType === PRICING_PLANS.proplan ? 'Go Pro' : 'Get Started'}
        ...
        cardFooter={<ul className='font-normal flex mb-2 flex-col gap-4'>
          ...
          {card.freatures.map((feature) => (
            <li key={feature} className='flex items-center gap-2'><Image src={CheckIcon} alt='Check Icon'/>
            {feature}
            </li>
          ))}
        </ul>}
