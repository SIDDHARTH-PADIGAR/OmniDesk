...
useEffect(() => {
  ...
  return () => {
    quill.off('text-change', selectionChangeHandler);
...
useEffect(() => {
  ...
  const subscription = room.on('presence', { event: 'sync'}, () => {
    ...
    room.track({
      ...
      avatarUrl: response.avatarUrl? supabase.storage.from('avatars').getPublicUrl(response.avatarUrl)
      .data.publicUrl
       : '',
    })
  })
 ...
 return (
    <>
      ...
      <div className='flex justify-center items-center flex-col mt-2 relative'>
        <div className='w-full self-center max-w-[800px] flax flax-col px-7 lg:my-8'>
            <div className='text-[80px]'>
                <EmojiPicker getValue={iconOnChange}>
                    <div className='w-[100px] cursor-pointer transition-colors h-[100px] flex items-center justify-center hover:bg-muted rounded-xl'>{details.iconId}</div>
                </EmojiPicker>
            </div>
            <div className="flex ">
            <BannerUpload
            ...
            variant="ghost" 
            className='gap-2 hover:bg-background flex items-center justify-center mt-1.5 text-sm text-muted-foreground w-36 p-2 rounded-md'>
              <XCircleIcon size={16}/>
              <span className='whitespace-nowrap font-normal'>Remove Banner</span>
              </Button>
              )}
            </div>
            <span className='text-muted-foreground text-3xl font-bold h-9' style={{ display: 'block' }}>
              {details.title}
            </span>
